...
public class TargetColoringScanner implements ITokenScanner {
	...
	private Map<String, IPartitionTokenScanner> coloringScanners;
	private IPartitionTokenScanner currentColoringScanner;
	...
	public TargetColoringScanner(Editor editor) {
		super();
		this.editor = editor;
		this.coloringScanners = new HashMap<String, IPartitionTokenScanner>();
	}
	 ...
	 * @return see above
	 */
	private IPartitionTokenScanner getColoringScanner(String partitionType) {
		IPartitionTokenScanner result = coloringScanners.get(partitionType);
		if (result == null) {
			ensureInitialized();
	 ...
	 */
	private IPartitionTokenScanner nextColoringScanner() {
		ensureInitialized();
		IToken token = partitionScanner.nextToken();
		if (token.isEOF()) {
			return partitionScanner;
		}
		IPartitionTokenScanner result = TargetColoringScanner.this
