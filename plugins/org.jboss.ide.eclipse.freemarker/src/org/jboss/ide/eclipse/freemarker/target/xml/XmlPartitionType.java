...
public enum XmlPartitionType {
	COMMENT(PreferenceKey.COLOR_XML_COMMENT) {
		...
		public IPredicateRule createPartitioningRule() {
			return new TargetMultiLineRule(XmlLexicalConstants.XML_COMMENT_START, XmlLexicalConstants.XML_COMMENT_END, new Token(this.name()), (char) 0, true);
		}
	},
	TAG(PreferenceKey.COLOR_XML_TAG) {
		...
		@Override
		public RuleBasedPartitionScanner createColoringTokenizer() {
			RuleBasedPartitionScanner result = super.createColoringTokenizer();
//			result.setRules(new IRule[] {
//			});
			return result;
		}
	...
	public RuleBasedPartitionScanner createColoringTokenizer() {
		...
		result.setDefaultReturnToken(PartitionType.createColoringToken(foregroundPreferenceKey));
		return result;
	}
	 ...
	 *
	 * @param name
	 * @return the {@link XmlPartitionType} that corresponds to the given {@code name}
